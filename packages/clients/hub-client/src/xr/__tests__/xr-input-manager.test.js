...
import { XRInputManager } from '../xr-input-manager';
// Mock XRSession
const createMockXRSession = (inputSources = []) => {
...
// Mock XRInputSource
const createMockInputSource = (handedness = 'none', gamepad) => {
    return {
        handedness,
...
// Mock Gamepad
const createMockGamepad = (id, buttons = [], axes = []) => {
    return {
        id,
...
// Mock XRFrame
const createMockXRFrame = (gripPose = null, aimPose = null) => {
    const frame = {
        getPose: vi.fn(),
    };
    frame.getPose.mockImplementation((space) => {
        if (gripPose && space === {})
            return gripPose; // gripSpace
        if (aimPose && space === {})
            return aimPose; // targetRaySpace
        return null;
    });
    return frame;
};
...
describe('XRInputManager', () => {
    ...
    describe('Haptic feedback', () => {
        ...
        it('should warn when triggering haptic on non-existent controller', () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith('[XRInputManager] Cannot trigger haptic: controller not found');
    ...
    describe('Controller connection/disconnection', () => {
        ...
        it('should remove controller state on disconnect', async () => {
            ...
            const removeMethod = xrInputManager.removeInputSource.bind(xrInputManager);
    ...
    describe('Button mappings', () => {
        it('should use correct button mapping for oculus-touch', async () => {
            const mockGamepad = createMockGamepad('Oculus Touch Controller', [
                { pressed: true, touched: true, value: 1 },
                { pressed: true, touched: true, value: 1 },
                { pressed: false, touched: false, value: 0 },
                { pressed: false, touched: false, value: 0 },
        ...
        it('should use default button mapping for unknown controller', async () => {
            const mockGamepad = createMockGamepad('Unknown Controller', [
                { pressed: true, touched: true, value: 1 },
            ], []);
            const leftController = createMockInputSource('left', mockGamepad);
            const session = createMockXRSession([leftController]);
    ...
    describe('Edge cases', () => {
        ...
        it('should handle update with no frame', async () => {
            ...
            await xrInputManager.initialize(session, mockReferenceSpace);
            expect(() => {
                xrInputManager.update(null, mockReferenceSpace);
            }).not.toThrow();
        });
