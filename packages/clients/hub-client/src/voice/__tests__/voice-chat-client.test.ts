...
class MockMediaStream implements MediaStream {
  ...
  constructor() {
    ...
    this.addTrack = vi.fn((track: MediaStreamTrack) => this.tracks.push(track));
    this.removeTrack = vi.fn((track: MediaStreamTrack) => {
...
(global as any).MediaStreamTrack = function() {
...
// Mock RTCTrackEvent
(global as any).RTCTrackEvent = class {
...
describe('VoiceChatClient', () => {
  ...
  function setupDefaultMocks() {
    ...
    const mockTrack = { ...mockMediaStreamTrack, enabled: true, stop: vi.fn() } as any;
    (mockStream as any).getAudioTracks.mockReturnValue([mockTrack]);
  ...
  describe('Connection', () => {
    ...
    it('should send offer to SFU', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        'https://sfu.example.com/sfu/rooms/test-room/peers',
        {
          method: 'POST',
          headers: {
  ...
  describe('Disconnection', () => {
    ...
    it('should stop local stream tracks', async () => {
      ...
      const mockStream = new MockMediaStream();
      const mockTrack = { ...mockMediaStreamTrack, enabled: true, stop: trackStopSpy } as any;
      (mockStream as any).getAudioTracks.mockReturnValue([mockTrack]);
      (mockStream as any).getTracks.mockReturnValue([mockTrack]);
      mockGetUserMedia.mockResolvedValue(mockStream as any);
  ...
  describe('Voice activity detection', () => {
    it('should detect voice activity', async () => {
      ...
      analyser.getByteFrequencyData.mockImplementation((data: Uint8Array) => {
  ...
  describe('Remote track handling', () => {
    it('should handle remote track event', async () => {
      ...
      (mockStream as any).getAudioTracks.mockReturnValue([mockAudioTrack]);
  ...
  describe('Signaling', () => {
    it('should handle remote offer', async () => {
      ...
      if (dc.onmessage) {
        await dc.onmessage({ data: JSON.stringify(offerMessage) } as MessageEvent);
    ...
    it('should handle unknown signaling messages gracefully', async () => {
      ...
      if (dc.onmessage) {
        dc.onmessage({ data: JSON.stringify({ type: 'unknown' }) } as MessageEvent);
      }

      expect(consoleSpy).toHaveBeenCalledWith(
        '[VoiceChat] Unknown signaling message:',
        'unknown'
      );
