 ...
 *
 * Creates a canvas texture that can be drawn on with various tools.
 * Supports multiple drawing modes: brush, line, shape, text
 */

export type DrawingMode = 'brush' | 'eraser' | 'line' | 'rectangle' | 'circle' | 'text';
export type DrawingColor = string;
export type DrawingTool = 'brush' | 'eraser' | 'fill' | 'line' | 'shape' | 'text';
...
export const DrawingCanvas = forwardRef<DrawingCanvasRef, DrawingCanvasProps>(
  ({ width = 1024, height = 1024, settings, onClear, onExport }, ref) => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const textureRef = useRef<THREE.CanvasTexture | null>(null);
    const isDrawingRef = useRef(false);
    const lastPositionRef = useRef<{ x: number; y: number } | null>(null);
    const pointsRef = useRef<Array<{ x: number; y: number }>>([]);
    const drawingHistoryRef = useRef<Array<ImageData>>([]);

    const [drawingHistory, setDrawingHistory] = useState<Array<ImageData>>([]);
    ...
    return (
      <group>
        ...
        <mesh>
          ...
          {textureRef.current && (
            <meshBasicMaterial map={textureRef.current} transparent side={THREE.DoubleSide} />
          )}
        </mesh>

        {/* Drawing canvas (off-screen) */}
        <canvas
          ref={canvasRef}
          style={{
            position: 'absolute',
