...
export class AvatarNameTag {
    constructor(displayName, config = {}) {
        const { showUserId = false, showStatus = true, backgroundColor = 'rgba(0, 0, 0, 0.7)', textColor = 'white', fontSize = 24, padding = 10, borderRadius = 8, } = config;
        ...
        // Store config
        this.sprite.config = { showUserId, showStatus, backgroundColor, textColor, fontSize, padding, borderRadius };
        this.sprite.displayName = displayName;
        // Initial render
     ...
     */
    update(displayName, status, isSpeaking) {
        this.sprite.displayName = displayName;
     ...
     */
    render(displayName, _status, isSpeaking) {
        const { ctx, canvas } = this;
        const config = this.sprite.config;
        // Clear canvas
     ...
     */
    setPosition(avatarPosition, avatarHeight) {
        this.sprite.position.set(avatarPosition.x, avatarPosition.y + avatarHeight + 0.3, avatarPosition.z);
    }
    /**
     * Update for camera
     */
