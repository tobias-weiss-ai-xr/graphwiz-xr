 ...
 */
export function useGrabSystem(grabbableObjects, myClientId, options = {}) {
    const { maxGrabDistance = 5, grabSmoothness = 0.2, throwMultiplier = 1.5, onGrab, onRelease, onHover, } = options;
    const { camera, pointer } = useThree();
    const raycaster = useRef(new Raycaster());
    const grabbedObject = useRef(null);
    const hoveredObject = useRef(null);
    const isGrabbing = useRef(false);
    /**
     * Check if an object can be grabbed by this client
     */
    const canGrabObject = useCallback((target) => {
     ...
     */
    const findGrabbableObject = useCallback(() => {
 ...
 */
export function useNetworkedGrabSystem(grabbableObjects, myClientId, sendGrabMessage, options = {}) {
